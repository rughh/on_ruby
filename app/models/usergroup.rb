class Usergroup
  ...
  def parse_recurring_date(date)
    ...
    if number == 'last'
      ...
      d.wday > day ? d + ((num + 1) * 7 + day - d.wday).days : d + (num * 7 + day - d.wday)
    end
  end
  ...
  def parse_recurring_time
    ...
    if time_string.present?
      ...
      Time.new(2_012, 5, 1, hour.to_i, min.to_i)
    else
      Time.new(2_012, 5, 1, 19, 0)
    end
  end
  ...
  def next_event_date
    ...
    unless d.future?
      d = parse_recurring_date(Date.today.next_month)
    end
  ...
  def localized_recurring
    if custom_recurring
      return localized_custom_recurrence
    end
  ...
  def localized_custom_recurrence
    ...
    if recurrence_text == 'n/a'
      recurrence_text = I18n.tw('custom_recurrence', locale: default_locale)
    end
  ...
  def self.switch_by_request(request)
    if Whitelabel.label_for(request.subdomains.first)
      return true
    end

    Whitelabel.label = Whitelabel.labels.find { |label| label.domains && label.domains.any? { |custom_domain| request.host =~ /#{custom_domain}/ } }
  end

  def self.from_name(name)
    unless name =~ /\A\w+\z/
      raise 'you need to provide a name that contains only word characters'
    end
    ...
    new.tap do |it|
      ...
      it.organizers = %w[your_twitter_handle]
