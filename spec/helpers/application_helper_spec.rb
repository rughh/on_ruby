...
describe ApplicationHelper do
  context 'render_cached' do
    ...
    it 'caches a scoped key' do
      allow(helper).to receive(:cache).with('hamburg/de/test/test', expires_in: 14_400, skip_digest: true)

      helper.render_cached
    end

    it 'caches passed keys' do
      allow(helper).to receive(:cache).with('hamburg/de/hallo/klaus', expires_in: 14_400, skip_digest: true)
